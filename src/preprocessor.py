...
class Preprocessor:
    ...
    def __init__(self,
        ...
        self.edge_list_paths = edge_list_paths
        self.config_name = config_name
        self.weight_type = weight_type
        self.save_weights = save_weights
        self.use_SVD = use_SVD
        self.SVD_dim = SVD_dim
        self.graphs = self._load(edge_list_paths, delimiter)
        self.union = self._get_union()

    def _load(self, paths, delimiter, scale=True):
        """
        """
        self.names = [path.split('/')[-1] for path in paths]
        graphs = [nx.read_weighted_edgelist(path, delimiter=delimiter).to_undirected()
    ...
    def _create_sparse_graphs(self):
        ...
        coo_mats = [np.abs(nx.to_scipy_sparse_matrix(G, nodelist=self.union, format='coo'))
        ...
        for mat in tqdm(coo_mats, desc='Creating sparse tensors'):
            ...
            idx, val = add_remaining_self_loops(idx, val)  # Ensure each node has a self-loop.
